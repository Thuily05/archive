...
**Câu hỏi trung tâm:** *"Are we going to reach our goals?"*
...
Để đọc thêm về story và thiết kế dashboard → [storyBoard.md](storyBoard.md)

---

## Ý tưởng tổng quan

Thay vì đặt KPI cảm tính, hệ thống:
...
4. **Gợi ý KPI** hợp lý theo tier (Achievable / Challenging / Stretch / Moonshot)
5. **Hiển thị toàn bộ** trên dashboard Streamlit tương tác
...
```
archive/
├── SampleSuperstore.csv          # Dữ liệu đầu vào
│
├── monthlyForecast2014.py        # Forecast chain — chạy theo thứ tự
├── monthlyForecast2015.py
├── monthlyForecast2016.py
├── monthlyForecast2017.py
├── suggestTarget.py              # Tổng hợp + tính tier ranges
│
├── target/
│   ├── target.py                 # KPI do con người đặt (dựa trên gợi ý model)
│   ├── suggestTargetConst.py     # Auto-generated constants
│   ├── suggestTargetResult.md    # Bảng gợi ý KPI range
│   └── suggestTargetChart.png    # Chart tier bands 2014–2018
│
├── expectKPIsOutput/             # Kết quả phân tích từng năm
│   ├── 2014/ … 2017/
│   │   ├── predictTarget.md      # Bảng rolling forecast + calibrated weights
│   │   └── predictTargetExplain.md
│   └── average/                  # Phân rã KPI theo Region / Category / Segment
│
├── src/
│   ├── app.py                    # Streamlit dashboard
│   └── requirements.txt
│
└── storyBoard.md                 # Story + design rationale của dashboard
```
...
monthlyForecast2014.py   ←─ Q1 actuals + XGBoost prior weights
        ...
        ▼
monthlyForecast2015.py   ←─ rolling forecast, so sánh actual vs target
...
monthlyForecast2017.py
        ...
        ▼
suggestTarget.py  ──→  target/suggestTargetResult.md
                  ──→  target/suggestTargetChart.png
                  ──→  target/suggestTargetConst.py
                            │
                            ▼
                      target/target.py  ←─ human sets KPI
                            │
                            ▼
...
- **Input:** `SampleSuperstore.csv` + Jan 2014 target thủ công (Sales=$50K, Profit=$5K, Customers=50, Orders=100)
- **Logic:**
...
- **Logic:** Giống 2014, không có given target — tất cả 12 tháng do model tự tính
- **Output:** Weights mới, annual target năm tiếp theo, forecast data

### suggestTarget.py
- **Input:** Import từ toàn bộ chain (2014→2017)
- **Logic (Prospective — nhìn từ đầu năm về phía trước):**
  - 2014: project annual từ Q1 actuals ÷ XGBoost seasonal weights
...
- **Output:** `suggestTargetResult.md`, `suggestTargetChart.png`, `suggestTargetConst.py`

### expectKPIsOutput/
Mỗi `predictTarget.md` gồm:
- **Q1 Projection detail** (chỉ 2014): cách suy ra annual baseline từ 3 tháng đầu
- **Rolling forecast table:** Target | Actual | Error% | Annual Estimate từng tháng
- **Year-End Comparison:** Predicted annual vs Sum monthly targets vs Final rolling estimate vs Actual
...
KPI được con người đặt dựa trên gợi ý của model (`target/target.py`):
...
Đọc thêm về thiết kế và story của dashboard → [storyBoard.md](storyBoard.md)
...
| `BASE_GROWTH` | 10% | Tăng trưởng nền để project annual năm tiếp theo |
...
| `matplotlib` | ≥3.8 | Xuất static PNG charts |
